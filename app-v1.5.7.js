(() => {
  ...
  /***************************************************************************
   * Meowmoon Bowling v1.5.7 cardinal-hanbok-ferry-animations
   * Adds the approved cardinal glide, cardinal fly-off, hanbok ribbon, and Mersey ferry animations.
   * Design: no choices, no score, no frames, no losing, no ads, no timers.
   **************************************************************************/

  console.log("Meowmoon Bowling code loaded: v1.5.7 cardinal-hanbok-ferry-animations");
  ...
  const SPECIAL_ANIMATION_REGISTRY = {
    ...
    cardinalglide: { label: "Cardinal Glide", duration: () => 4200, updateMode: "staticTimed", overlay: true, draw: cardinalv2DrawPin },
...
function drawUnderarmCrutchesWalkPin(pin, current) {
  ...
  // =====================================================
  function cardinalv2Clamp(value, min, max) { return Math.max(min, Math.min(max, value)); }
  function cardinalv2Lerp(a, b, t) { return a + (b - a) * t; }
  function cardinalv2EaseInOut(t) { return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2; }
  function cardinalv2EaseOut(t) { return 1 - Math.pow(1 - t, 3); }
  function cardinalv2DrawStar(ctx, x, y, r, color) {
    ctx.save();
    ctx.translate(x, y);
    ctx.fillStyle = color;
    ctx.beginPath();
  ...
  function cardinalv2Draw(ctx,x,y,scale,elapsed,duration){
    const p=cardinalv2Clamp(elapsed/duration,0,1);
    const fly=cardinalv2Clamp(p/0.82,0,1);
    const startX = x-336*scale;
    const endX = x-16*scale;
    const cx=cardinalv2Lerp(startX,endX,cardinalv2EaseInOut(fly));
    const cy=cardinalv2Lerp(y-130*scale,y-12*scale,fly)-Math.sin(fly*Math.PI)*26*scale;
    const wing=Math.sin(Math.min(1,p/0.8)*Math.PI*8)*0.45;
    ctx.save(); ctx.strokeStyle='#6b4f37'; ctx.lineWidth=5*scale; ctx.beginPath(); ctx.moveTo(x+44*scale,y+18*scale); ctx.lineTo(x+170*scale,y+18*scale); ctx.stroke(); ctx.restore();
    ...
    if(p>0.82){ cardinalv2DrawStar(ctx,x+150*scale,y-56*scale,8*scale,'#ffd36a'); }
  }
  ...
  function cardinalv2DrawPin(pin, current) {
    const elapsed = current - pin.rocket.startedAt;
    const scale = Math.min(view.w / 780, view.h / 560) * 2;
    const anchorX = clamp(pin.x, 60 * scale, view.w - 175 * scale);
    const anchorY = pin.y + layout.pinH * 0.58;
    cardinalv2Draw(ctx, anchorX, anchorY, scale, elapsed, pin.rocket.duration);
  }
