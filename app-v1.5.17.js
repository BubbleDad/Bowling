(() => {
  ...
  /***************************************************************************
   * Meowmoon Bowling v1.5.17 cardinalflyoff-top-safe-area
   * Keeps Cardinal Flies Off Screen v1 launch pose visible near the top edge.
   * Design: no choices, no score, no frames, no losing, no ads, no timers.
   **************************************************************************/

  console.log("Meowmoon Bowling code loaded: v1.5.17 cardinalflyoff-top-safe-area");
...
function drawUnderarmCrutchesWalkPin(pin, current) {
  ...
  function friendlystarshipglidev1DrawPin(pin, current) {
    ...
    const marginX = 260 * scale;
    const anchorX = clamp(pin.x, marginX, view.w - marginX);
    const anchorY = clamp(pin.y + layout.pinH * 0.20, 180 * scale, view.h - 150 * scale);
