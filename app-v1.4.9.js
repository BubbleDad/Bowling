(() => {
  ...
  console.log("Meowmoon Bowling code loaded: v1.4.9 transparent-start-overlay");
  ...
  function animationTypesByExclusiveGroup(groupName) {
    return PIN_SPECIAL_TYPES.filter(type => animationSpec(type)?.exclusiveGroup === groupName);
  }
...
function drawFireTruckPin(pin, current) {
  ...
  function drawEmergencyStartScreen() {
    try {
      ...
      ctx.fillStyle = "#34308b";
      ctx.fillText("Tap anywhere to roll", view.w / 2, view.h * 0.51);
    } catch (err) {
